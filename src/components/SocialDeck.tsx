...
const getTranslatedCardField = (id: string, field: 'desc' | 'count', locale: 'en' | 'zh', defaultValue: string) => {
  const translationsMap: Record<string, Record<'desc' | 'count', Record<'en' | 'zh', string>>> = {
    bilibili: {
      desc: { en: "Video tutorials & interactive motion showcases", zh: "流体交互视频与教程发布" },
      count: { en: "128k Followers", zh: "12.8万 关注者" }
    },
    weibo: {
      desc: { en: "Daily design inspiration & aesthetic sharing", zh: "日常设计灵感与美学分享" },
      count: { en: "56k Followers", zh: "5.6万 粉丝" }
    },
    github: {
      desc: { en: "Open-source 3D animations & UI motion libraries", zh: "开源前端三维动画与动效库" },
      count: { en: "3.2k Stars", zh: "3.2k Stars" }
    },
    youtube: {
      desc: { en: "Immersive motion design video archives", zh: "精美动效沉浸式视频档案室" },
...
const getBrandIcon = (id: string) => {
  switch (id.toLowerCase()) {
    case 'bilibili':
      return (
        <svg className="w-8 h-8 md:w-9 md:h-9 shrink-0" viewBox="0 0 24 24" fill="currentColor">
    ...
    case 'instagram':
      return (
        <svg className="w-8 h-8 md:w-9 md:h-9 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    default:
      return (
        <svg className="w-8 h-8 md:w-9 md:h-9 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
export default function SocialDeck({
  ...
  const displayedLinks = isEditingMode ? socialLinks : socialLinks.filter(l => l.active);
  ...
  const renderLinks = isEditingMode || isExpanded ? displayedLinks : displayedLinks.slice(0, maxInitialLinks);
  ...
  const handleSaveLink = () => {
    ...
    if (isAddingNew) {
      ...
      newLinks = socialLinks.map(l => l.id === editingLink.id ? updatedLink : l);
  ...
  return (
    <div className="w-full select-none flex flex-col h-full">
      ...
      <div className={`relative flex-1 flex flex-col min-h-0 p-5 md:p-6 rounded-[2.5rem] border-4 shadow-[8px_8px_0px_#0f172a] overflow-hidden ${
        isEditingMode ? 'border-dashed border-brand-orange/60 bg-brand-orange/5' : ''
      } ${
        theme === 'dark' 
        ...
        <div className="flex-1 min-h-0 overflow-y-auto no-scrollbar relative z-10 py-4 w-full h-full">
          <div className={`grid ${gridColsClass} content-start justify-items-center gap-4 w-full`}>
            {renderLinks.map((link) => (
              <div key={link.id} className="relative group/card w-full max-w-[80px]">
          ...
          <div className="relative z-10 mt-3 pt-3 border-t-2 border-slate-900 border-dashed flex justify-center">
            <button 
              ...
              className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-2 py-2 px-4 rounded-xl border-2 border-slate-900 shadow-[2px_2px_0px_#0f172a] transition-all hover:-translate-y-0.5 hover:shadow-[4px_4px_0px_#0f172a] ${
                theme === 'dark' ? 'bg-slate-900 text-white hover:bg-slate-900' : 'bg-slate-50 text-slate-900 hover:bg-white'
              }`}
            >
              {isExpanded ? (
                <>
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="18 15 12 9 6 15"></polyline></svg>
                ...
                <>
                  ...
                  {locale === 'zh' ? `展示更多 (${activeLinksCount - maxInitialLinks})` : `Show More (${activeLinksCount - maxInitialLinks})`}
                </>
              )}
            </button>
          </div>
      ...
      <AnimatePresence>
        {selectedCard && !isEditingMode && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className={`absolute top-0 inset-x-0 h-2 bg-gradient-to-r ${selectedCard.color}`} />

              <button 
                onClick={() => { playClickSound(); setSelectedCard(null); }}
              ...
              <div className="flex flex-col items-center text-center mt-3 space-y-6">
                <div className={`w-20 h-20 rounded-3xl bg-gradient-to-tr ${selectedCard.color} text-white flex items-center justify-center shadow-[6px_6px_0px_#0f172a] border-4 border-slate-900`}>
                ...
                <div>
                  <div className="flex items-center justify-center gap-2">
                    <h4 className="text-xl font-black font-display uppercase tracking-tighter">{selectedCard.name}</h4>
                    <span className="py-1 px-2 rounded-lg bg-emerald-100 text-emerald-700 border-2 border-slate-900 shadow-[2px_2px_0px_#0f172a] text-[10px] font-black uppercase tracking-widest flex items-center gap-1">
                      <CheckCircle2 className="w-3 h-3 fill-current" /> {t.social_deck_verified}
                    </span>
                  </div>
                  <p className="text-xs text-brand-orange font-black uppercase tracking-widest mt-2">{selectedCard.username}</p>
                </div>

                <div className={`p-4 rounded-[2rem] border-2 w-full ${
                  theme === 'dark' ? 'bg-slate-900 border-slate-900 shadow-[inset_4px_4px_12px_rgba(0,0,0,0.5)]' : 'bg-slate-50 border-slate-900 shadow-[inset_4px_4px_12px_rgba(0,0,0,0.05)]'
                }`}>
                  <div className="w-40 h-40 flex items-center justify-center relative bg-white border-2 border-slate-900 p-3 rounded-2xl shadow-[6px_6px_0px_#0f172a] mx-auto">
                    ...
                    <div className={`absolute inset-0 m-auto w-12 h-12 rounded-xl bg-gradient-to-tr ${selectedCard.color} border-2 border-slate-900 text-white flex items-center justify-center shadow-[3px_3px_0px_#0f172a] scale-95`}>
                ...
                <div className="space-y-2 px-4">
                  <p className={`text-sm font-bold leading-relaxed ${
                    ...
                    {selectedCard ? getTranslatedCardField(selectedCard.id, 'desc', locale, selectedCard.desc) : ''}
                  </p>
                  <p className="text-xs font-black uppercase tracking-widest text-brand-orange">
                    {selectedCard ? getTranslatedCardField(selectedCard.id, 'count', locale, selectedCard.count) : ''}
                  </p>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className={`relative w-full max-w-md p-6 rounded-[2rem] border-4 shadow-[12px_12px_0px_#0f172a] overflow-hidden z-50 ${
                theme === 'dark' ? 'bg-slate-800 border-slate-900 text-white' : 'bg-white border-slate-900 text-slate-900'
              }`}
            >
              <div className={`absolute top-0 inset-x-0 h-2 bg-gradient-to-r ${formColor}`} />
              ...
              <div className="space-y-4 text-left">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (lower.includes('bilibili') || lower.includes('哔哩')) {
                        setFormColor('from-pink-400 to-pink-500');
                      } else if (lower.includes('weibo') || lower.includes('微博')) {
                      ...
                      } else if (lower.includes('tiktok') || lower.includes('抖音')) {
                        setFormColor('from-indigo-500 to-purple-600');
                      }
                    }}
                    placeholder="如 Bilibili, Weibo, GitHub"
                    className="w-full px-3 py-2 rounded-xl border-2 border-slate-900 text-xs font-bold focus:outline-none focus:border-brand-orange text-slate-800 bg-white"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 rounded-xl border-2 border-slate-900 text-[10px] text-slate-500 font-medium focus:outline-none focus:border-brand-orange bg-white"
                  />
                </div>
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1">
                    简介描述 (Card Description)
                  </label>
                  <input
                    type="text"
                    value={formDesc}
                    onChange={(e) => setFormDesc(e.target.value)}
                    placeholder="例如: 流体交互视频与教程发布"
                    className="w-full px-3 py-2 rounded-xl border-2 border-slate-900 text-xs font-bold focus:outline-none focus:border-brand-orange text-slate-800 bg-white"
                  />
                </div>
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1">
                    关注数指标 (Stats Count Text)
                  </label>
                  <input
                    type="text"
                    value={formCount}
                    onChange={(e) => setFormCount(e.target.value)}
                    placeholder="例如: 12.8万 关注者"
                    className="w-full px-3 py-2 rounded-xl border-2 border-slate-900 text-xs font-bold focus:outline-none focus:border-brand-orange text-slate-800 bg-white"
                  />
                </div>

                {/* 6. Active State Toggle (Show or hide specific social media icon) */}
                <div className="flex items-center justify-between p-2.5 bg-slate-50 border-2 border-slate-900 rounded-xl">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-slate-700">显示此社交图标 (Show on Deck)</span>
                  </div>
                  <input 
                    type="checkbox"
                    checked={formActive}
                ...
                {/* 7. Color Gradients Presets */}
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1.5">
                    卡片背景主题色 (Brand Theme Gradient)
                  </label>
                  <div className="grid grid-cols-4 gap-1.5">
                    {COLOR_PRESETS.map((p) => (
                      <button
                        ...
                        className={`py-1.5 px-1 rounded-lg text-[9px] font-black border-2 text-center text-white bg-gradient-to-tr ${p.value} transition ${
                          formColor === p.value ? 'border-slate-900 shadow-[1px_1px_0px_#000]' : 'border-slate-200'
                        }`}
                      >
                        {p.name}
                      </button>
                ...
                {/* Action buttons */}
                <div className="flex gap-2.5 pt-4 border-t border-slate-100 border-dashed">
                  {!isAddingNew && (
                    <button
                      type="button"
                      onClick={() => handleRemoveLink(editingLink.id, formName)}
                      className="px-4 py-2 bg-red-50 text-red-500 border-2 border-slate-900 hover:bg-red-100 font-black text-xs uppercase tracking-wider rounded-xl shadow-[2px_2px_0px_#0f172a] hover:shadow-none transition-all cursor-pointer flex items-center gap-1.5"
                    >
                  ...
                  <div className="flex-1 flex gap-2 justify-end">
                    <button
                      type="button"
                      onClick={() => setEditingLink(null)}
                      className="px-4 py-2 bg-white text-slate-700 border-2 border-slate-900 hover:bg-slate-50 font-black text-xs uppercase tracking-wider rounded-xl transition-all cursor-pointer"
                    >
                      取消
                    </button>
                    <button
                      ...
                      className="px-4 py-2 bg-brand-orange text-white border-2 border-slate-900 font-black text-xs uppercase tracking-wider rounded-xl shadow-[2px_2px_0px_#0f172a] hover:shadow-none hover:translate-x-0.5 hover:translate-y-0.5 transition-all cursor-pointer flex items-center gap-1.5"
                    >
...
function SocialCard({
  ...
  return (
    ...
    >
      ...
      <div className={`w-12 h-12 md:w-14 md:h-14 rounded-xl border-2 border-slate-900 bg-gradient-to-tr ${link.color} text-white flex items-center justify-center shadow-[2px_2px_0px_#0f172a] transition-transform duration-300 ${
